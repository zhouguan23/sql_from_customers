select A.*,B.area_name from DM_STOCK_GENERAL A inner join DIM_REGION B ON A.AREA_CODE=B.AREA_CODE
where 1=1 ${if(len(region)=0,"", "and A.Area_code in ('"+region+"')")}  and ddate=last_day(to_date('${Date}'||'-01','yyyy-mm-dd'))
...
sum(nvl(dckc,0))/(sum(nvl(zycb,0))+sum(nvl(jmcb,0))+sum(nvl(pfcb,0))+sum(nvl(glcb,0)))
