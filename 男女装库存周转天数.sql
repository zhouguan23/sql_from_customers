...
set @sql1='
	...
	left(a.GoodsCatCode,3)GoodsCatCode1,case when left(a.GoodsCatCode,3)=2 then 2 else  a.GoodsCatCode end GoodsCatCode ,
	isnull(sum(b.SaleCost),0)SaleCost ,isnull(sum(a.StockCost),0)Cost from 
	(select DepartmentCode nodecode,left(CategoryCode,4)GoodsCatCode, SUM(StockCost)StockCost from 
...
group by DepartmentCode,left(CategoryCode,4)
) a 
	left join 
	
	(select nodecode,GoodsCatCode,sum(SaleCost)SaleCost from 
	
	(select DepartmentCode nodecode,left(CategoryCode,4)GoodsCatCode,SUM(CommonSaleCost+PromotionSaleCost) SaleCost 
...
group by DepartmentCode,left(CategoryCode,4))b
...
and   1=1 ${if(len(dq) == 0,   "",   "and d.DeptCategoryCode in (''" + replace(dq,",","'',''")+"'')") }
	group by d.DeptCategoryCode,d.CategoryName,a.nodecode,left(a.GoodsCatCode,3),case when left(a.GoodsCatCode,3)=2 then 2 else  a.GoodsCatCode end
    order by 1,3,5,6
	'exec (@sQL1)
