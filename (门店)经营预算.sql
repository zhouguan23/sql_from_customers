...
WHERE type = 'p'
	...
	sum(ISNULL(D.Grossprofitindex,0))Grossprofitindex,sum(ISNULL(c.Gross,0)+isnull(e.kx,0))Gross,sum(ISNULL(c.AfterGross,0))AfterGross
	 from 
	(select b.YM,a.NodeCode,a.CategoryCode ,a.CategoryName ,a.bm  from 
	(select  b.NodeCode ,a.CategoryCode ,a.CategoryName ,c.CategoryCode bm
	from 
	dbo.TB部门分类对照表 a 
	left join 
    ...
    group by DeptCode,CategoryCode,BudgetYM) b on a.bm=b.CategoryCode and a.YM=b.BudgetYM and a.NodeCode=b.DeptCode 	       
    	    left join 
	    (select DeptCode,CategoryCode,BudgetYM,sum(Salesindex)Salesindex,sum(Grossprofitindex) Grossprofitindex
     from .[无税分课预算表]A
    where CategoryItemCode ='0000' 
    group by DeptCode,CategoryCode,BudgetYM) d on a.bm=d.CategoryCode and a.YM=d.BudgetYM and a.NodeCode=d.DeptCode 	     
    left join 
        ...
        where 1=1 ${if(len(bm) == 0,   "",   "and a.nodecode in ('" + replace(bm,",","','")+"')") } 
	    group by a.CategoryCode,a.CategoryCode ,a.CategoryName,YM,a.NodeCode
...
from TB部门分类对照表
where CategoryItemCode='0000' and CategoryLevel='1' and DeptCpde = (select FormatCode  from TB部门信息表 a where a.nodecode ='${bm}')
