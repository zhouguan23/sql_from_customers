...
dxamt_plan,qty_plan/250/days avgqtyplan
...
select cc.sale_dept_id,sum(cl.qty_ord*t_size)/50000 qtyday ,round(sum(qty_ord*price)*50000/sum(cl.qty_ord*t_size),0) dxamtday,sum(qty_ord*price) amt from co_co cc,co_co_line cl,plm_item i
where cc.co_num=cl.co_num and cl.item_id=i.item_id 
and cc.born_date=(select to_char(sysdate,'yyyyMMdd') from dual) and cc.status<>'90'
group by cc.sale_dept_id

select '1137'||substr(dpt_sale_id,1,4) sale_dept_id,sum(qty_sold*t_size)/50000 qtymonth,
...
select trunc(sysdate)-to_date('20150101','yyyymmdd')+1 daypast from dual
