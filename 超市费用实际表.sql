...
order by 2,4,5
    ...
    a.ParentCategoryCode,a.CategoryCode,a.CategoryName, sum(isnull(b.salesindex,0))salesindex ,sum(isnull(b.salesindexcw,0))salesindexcw,sum(isnull(b.salesindexkj,0))salesindexkj ,sum(isnull(b.salesindextz,0))salesindextz  ,sum(isnull(b.salesindexyuti,0))salesindexyuti, sum(isnull(b.xscwdd,0))
xscwdd, sum(isnull(b.xskjtz,0))xskjtz , sum(isnull(b.xsihtz,0))xsihtz , sum(isnull(b.xsyuti,0))xsyuti, sum(isnull(b.sdsys,0))sdsys, sum(isnull(b.sdscw,0))sdscw , sum(isnull(b.sdskjtz,0))sdskjtz , sum(isnull(b.sdstz,0))sdstz, sum(isnull(b.sdsyuti,0))sdsyuti, sum(isnull(b.zzsys,0))zzsys, sum(isnull(b.zzscw,0))zzscw , sum(isnull(b.zzskjtz,0))zzskjtz , sum(isnull(b.zzstz,0))zzstz , sum(isnull(b.zzsyuti,0))zzsyuti
	 from 
...
WHERE type = 'p'
    ...
    --获取部门
        ,TB部门信息表 c
        where (left(c.NodeCode,1) between 1 and 2 or left(c.NodeCode,1) in('9','5') ) 
        and  a.CategoryLevel='2'         and  1=1 ${if(len(bm) == 0,   "",   "and c.NodeCode in ('" + replace(bm,",","','")+"')") } 
and  a.CategoryItemCode ='9001'
)a
        left join 
        TB费用跟踪 b on a.NodeCode=b.deptcode and a.YM=b.budgetYM and a.CategoryCode=b.CategoryCode and   1=1 ${if(len(lx) == 0,   "",   "and b.CategoryItemCode in ('" + replace(lx,",","','")+"')") }
...
select a.NodeCode,a.NodeName,a.YM,a.ParentCategoryCode,a.CategoryCode,a.CategoryName,isnull(b.salesindex,0)salesindex ,isnull(b.salesindexcw,0)salesindexcw ,isnull(b.salesindexkj,0)salesindexkj ,isnull(b.salesindextz,0)salesindextz ,isnull(b.salesindexyuti,0)salesindexyuti   from 
    (select c.NodeCode,c.NodeName,b.YM,a.CategoryCode,a.CategoryName,a.ParentCategoryCode  from 
    --获取分类
TB预算科目 a,
--获取年月
...
WHERE type = 'p'
    ...
    --获取部门
        ...
        and  1=1 ${if(len(bm) == 0,   "",   "and c.NodeCode in ('" + replace(bm,",","','")+"')") } )a
        left join 
           tB营业外收入实际表 b on a.NodeCode=b.deptcode and a.YM=b.budgetYM and a.CategoryCode=b.CategoryCode and   1=1 ${if(len(lx) == 0,   "",   "and b.CategoryItemCode in ('" + replace(lx,",","','")+"')") } 
    order by a.NodeCode,a.YM ,Convert(int,left(a.CategoryCode,charindex('.',a.CategoryCode+'.')-1))+0 ,Convert(int,left(a.parentcategorycode,charindex('.',a.parentcategorycode+'.')-1))+0
...
and  a.CategoryItemCode ='9001'
        ...
        left join 
        TB费用预算 b on a.NodeCode=b.deptcode and a.YM=b.budgetYM and a.CategoryCode=b.CategoryCode and   1=1 ${if(len(lx) == 0,   "",   "and b.CategoryItemCode in ('" + replace(lx,",","','")+"')") }
        group by a.NodeCode,a.NodeName,a.YM,case when a.ParentCategoryCode <=13 then '经营费用' when a.ParentCategoryCode in (14,15,16,17,18) then '不可控费用' end, a.ParentCategoryCode,a.CategoryCode,a.CategoryName
    order by a.NodeCode,a.YM ,Convert(int,left(a.CategoryCode,charindex('.',a.CategoryCode+'.')-1))+0 ,Convert(int,left(a.parentcategorycode,charindex('.',a.parentcategorycode+'.')-1))+0
