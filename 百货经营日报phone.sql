...
含税分课预算表
where(DeptCode   LIKE '1%' OR DeptCode   LIKE '2%' OR DeptCode   LIKE '9%')  and CategoryItemCode='0000' and DeptCode !='9999'
...
isnull(d.CarryState,0)CarryState
 from 
(select  AreaCode,NodeCode ,convert(varchar(10),dateadd(dd,number,convert(varchar(8),dateadd(dd,-day('${dqrq}')+1,'${dqrq}'),112)),112)defday
from master..spt_values a ,TB部门信息表 b
where type='p' and number <= datediff(dd,dateadd(dd,-day('${dqrq}')+1,'${dqrq}')   ,dateadd(ms,-3,DATEADD(mm, DATEDIFF(m,0,'${dqrq}')+1, 0)))
and (NodeCode  LIKE '9%') and len(AreaCode)>0  and NodeCode !='9999' and nodecode  not like '9029'
...
left join 
TBDAYCARRYLOG d on a.NodeCode=d.deptcode and a.defday=d.CarryDate
where a.defday=convert(varchar(8),dateadd(dd,0,'${dqrq}'),112) 
and NodeCode!='9029'
group by AreaCode,NodeCode ,a.defday,isnull(d.CarryState,0)
