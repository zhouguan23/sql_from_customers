...
group by b.union_area_name,m.large_cate,m.small_cate
order by sorted,m.large_cate,m.small_cate

select r.union_area_name,(case when '${Tax}'='无税' then  sum(no_tax_amount) else  sum(tax_amount) end) 直营非DTP销售
from dm_monthly_company a,dim_region r
