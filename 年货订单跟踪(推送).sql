...
left join 
(select deptcode,goodscode,sum(Amount)Amount,sum(PurchMoney)PurchMoney from [hldw]A.dbo.tbGoodsArrivalannual where LastPerformDate between 20191209 and 20200124 group by deptcode,goodscode) f on a.deptcode=f.deptcode and a.goodscode=f.goodscode
