...
(select distinct a.DeptCode,a.GoodsCode,b.YM,a.UseBegMonth from
(select distinct c.DeptCode,b.GoodsCode,a.UseBegMonth from 
TB分类对照表 a
left join 
...
WHERE type = 'p'
    ...
    AND CONVERT(varchar(6),DATEADD(month,number,'${qsrq}'+'0101'),112)>='201906')b
    where a.deptcode is not null and b.YM>=a.UseBegMonth
    )a
left join 
...
select * from 
TB营业外收入预算表 a
where exists(select * from TB分类对照表 z where a.deptcode=z.CategoryCode and  z.CategoryItemCode='0003' and z.CategoryLengCode like '${bm}%' and a.BudgetYM>=z.UseBegMonth) and BudgetYM like '${qsrq}%'
...
TB部门费用预算表 a
where 
exists(select * from TB分类对照表 z where a.deptcode=z.CategoryCode and  z.CategoryItemCode='0003' and z.CategoryLengCode like '${bm}%' and a.BudgetYM>=z.UseBegMonth)
and BudgetYM like '${qsrq}%'
...
TB费用报销单 a
where BuildDate like '${qsrq}%' and  exists(select * from TB分类对照表 z where a.ChargeDeptCode=z.CategoryCode and  z.CategoryItemCode='0003' and z.CategoryLengCode like '${bm}%' and a.BuildYM>=z.UseBegMonth) and AdaltState in ('1','2')
...
where a.BuildDate like '${qsrq}%' and  exists(select * from TB分类对照表 z where a.ChargeDeptCode=z.CategoryCode and  z.CategoryItemCode='0003' and z.CategoryLengCode like '${bm}%' )  and case when len(c.TEXT_)!=0 then c.TEXT_ else a.AdaltState end not in ('-1','7','9')
group by ChargeDeptCode,CostCoding,BuildYM
...
(select a.DeptCode,a.GoodsCode,b.YM,a.UseBegMonth from
(select distinct c.DeptCode,b.GoodsCode,a.UseBegMonth from 
TB分类对照表 a
left join 
tbCatToGoods b on a.CategoryItemCode=b.CategoryItemCode and a.CategoryCode=b.CategoryCode
...
(select distinct a.DeptCode,left(a.GoodsCode,1) GoodsCode,b.YM,a.UseBegMonth from
(select distinct c.DeptCode,b.GoodsCode,a.UseBegMonth from 
TB分类对照表 a
left join 
tbCatToGoods b on a.CategoryItemCode=b.CategoryItemCode and a.CategoryCode=b.CategoryCode
