...
from  plm_brandowner@hzyx f,plm_item  i,  item_com  ic ,plm_brand@hzyx pp

where f.brdowner_id=i.brdowner_id  and pp.brand_id=i.brand_id
and i.item_id=ic.item_id and  i.is_mrb=1   
...
select  a.item_id,round(sum(a.qty_sold*t_size)/50000,2) qty,round(sum(a.amt_sold),0) amt,round(sum(a.gross_profit),0) ml
 ...
 where  a.item_id=i.item_id and sale_dept_id like '${dptid}%' and date1>=(select to_char(to_date('${date1}','yyyymmdd'),'yyyyMM')||'01' from dual)
...
and sale_dept_id like '${dptid}%' and date1>=(SELECT SUBSTR(TO_CHAR(add_months(to_date('${date1}','yyyyMMdd'),-12),'yyyyMMdd'),1,6)||'01' FROM DUAL) and  date1<=(SELECT TO_CHAR(add_months(to_date('${date1}','yyyyMMdd'),-12),'yyyyMMdd') FROM DUAL) 
