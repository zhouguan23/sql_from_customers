...
and b.CategoryLevel=2  and b.CategoryItemCode='0000' and CategoryCode  in (10,11,12,13,14)
)a
LEFT JOIN 
含税分课预算表 B ON A.NodeCode=b.DeptCode and a.GoodsCatcode=B.CategoryCode 
and BudgetYM =convert(varchar(6),dateadd(dd,0,'${dqrq}'),112) and CategoryItemCode in ('0000')and DeptCode not in (0002)
...
TB部门信息表 A,
 ('+@SQL+')  B
WHERE A.NodeCode=b.部门编码 and  (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' )  AND 商品分类编码 LIKE ''1%''
...
TB部门信息表 A,
 ('+@SQL+')  B
WHERE A.NodeCode=b.DeptCode  and  (nodecode LIKE ''1%'' or nodecode LIKE ''1%'') and goodscode like ''1%''
