...
from (
select a.area_code,C.disable_code as goods_code,date1,dtp,jcsx,a.goods_name,a.manufacturer,a.specification,
...
1=1 
${if(len(gcode)=0,""," and c.disable_code in ('"+gcode+"')")}
and date1='${date}'
and 1=1
${if(len(dtp)=0,""," and dtp = '"+dtp+"'")} and 1=1
${if(len(gather)=0,""," and jcsx in ('"+gather+"')")}
group by a.area_code,C.disable_code,date1,dtp,jcsx,a.goods_name,a.manufacturer,a.specification,b.CATEGORY,b.SUB_CATEGORY ) a,
(select count(1) as DAYS from DIM_DAY where MONTH_ID='${date}') b
      )sale_data
...
select distinct c.disable_code as goods_code,c.disable_code||'|'||b.goods_name goods_name from  dim_goods b   join dim_disable_code C on b.goods_code=c.disable_code
order by 1 
