...
select ParentCategoryCode,a.CategoryCode ,a.CategoryName,YM month,a.NodeCode DeptCode ,b.非中央控制,b.中央控制,b.门店缺货
 from 
(select ParentCategoryCode,CategoryCode ,CategoryName,YM,a.NodeCode  from 
(select  '0002' NodeCode,a.ParentCategoryCode,a.CategoryCode ,a.CategoryName 
from dbo. TB考核分类表 a  where CategoryItemCode='0002' and CategoryLevel=1 and left(CategoryCode,1) between 3 and 5
...
WHERE type = 'p'
    ...
    left join 
    dbo.[tb缺货率指标预算表]A b on a.CategoryCode=b.CategoryCode and a.YM=b.BudgetYM and a.NodeCode=b.DeptCode and   b.CategoryItemCode ='0002'
    order by 2,4,6,5,7
...
select LEFT(a.CategoryCode,1)Category1Code,case when LEFT(a.CategoryCode,2)='39' then '30' else LEFT(a.CategoryCode,2) end  Category2Code ,sum(可订货次数)可订货次数,sum(非中央控制缺货次数)+sum(中央控制缺货次数)+sum(门店缺货次数)缺货次数,
sum(非中央控制缺货次数)非中央控制缺货次数,sum(中央控制缺货次数)中央控制缺货次数,sum(门店缺货次数)门店缺货次数 from  
TB${YM}_月度缺货数据 a 
GROUP BY LEFT(a.CategoryCode,1),case when LEFT(a.CategoryCode,2)='39' then '30' else LEFT(a.CategoryCode,2) end
order by  2
