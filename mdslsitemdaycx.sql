...
sum(a.qty_sold*i.t_size)/50000 qty from  plm_item i ,pi_cust_item_day a  ,co_cust cc
...
select i.item_id,item_name,cc.slsman_id,i.brdowner_id,
sum(a.qty_sold*i.t_size)/50000 qtyt from  plm_item i ,pi_cust_item_month a  ,co_cust cc
 where i.item_id=a.item_id    and a.cust_id=cc.cust_id 
  and cc.slsman_id in ('${sls}') 
 and i.item_id in ('${itemid}')

and date1>=(select to_char(add_months(to_date('${date1}','yyyymmdd'),-12),'yyyymmdd' ) from dual)
 and date1<=(select to_char(add_months(to_date('${date2}','yyyymmdd'),-12),'yyyymmdd' ) from dual)
group by i.item_id,item_name,cc.slsman_id,i.brdowner_id

select i.item_id,item_name,cc.slsman_id,i.brdowner_id,
sum(a.qty_sold*i.t_size)/50000 qtys from  plm_item i ,pi_cust_item_day a  ,co_cust cc
 where i.item_id=a.item_id    and a.cust_id=cc.cust_id 
  and cc.slsman_id in ('${sls}') 
 and i.item_id in ('${itemid}')

and date1>=(select to_char( to_date('${date1}','yyyymmdd')-(to_date('${date2}','yyyymmdd')-to_date('${date1}','yyyymmdd')+1),'yyyymmdd') from dual)
