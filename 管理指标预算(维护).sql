...
from dbo.TB商品分类表 a  where CategoryItemCode='0000' and CategoryLevel=2 and left(CategoryCode,1) between 3 and 5
...
select LEFT(a.CategoryCode,1)Category1Code,LEFT(a.CategoryCode,2)Category2Code ,sum(可订货次数)可订货次数,sum(非中央控制缺货次数)+sum(中央控制缺货次数)+sum(门店缺货次数)缺货次数,
sum(非中央控制缺货次数)非中央控制缺货次数,sum(中央控制缺货次数)中央控制缺货次数,sum(门店缺货次数)门店缺货次数 from  
TB${YM}_月度缺货数据 a 
GROUP BY LEFT(a.CategoryCode,1),LEFT(a.CategoryCode,2)
order by  2
