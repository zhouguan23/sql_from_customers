...
,subtab.ADJ_CONT_AMT
,subtab.ADJ_CONT_CN
,subtab.APPROVAL_ADJCONT_CN
,subtab.APPROVAL_ADJCONT_AMT
...
sum(ADJ_CONT_CN) as ADJ_CONT_CN,
sum(APPROVAL_ADJCONT_CN) as APPROVAL_ADJCONT_CN,
sum(APPROVAL_ADJCONT_AMT) as APPROVAL_ADJCONT_AMT
from f_cost_adjcont
where 1=1 
and area_org_code in(
select distinct b.AREA_ORG_CODE from user_org a  left join fr_po_priv b on a.SAP_DEPT_ID=b.proj_code
...
select distinct CONT_CLASSNAME1 from f_cost_adjcont
where 1=1 and CONT_CLASS1 in (select sap_DEPT_ID from user_org where ORG_TYPE='合同一级分类' 
)

select distinct CONT_CLASSNAME2  from f_cost_adjcont
where 1=1
${if(len(CONT_CLASSNAME1) == 0,"","and CONT_CLASSNAME1 in ('" + CONT_CLASSNAME1 + "')")}

select distinct CONT_CLASSNAME3  from f_cost_adjcont
where 1=1
${if(len(CONT_CLASSNAME1) == 0,"","and CONT_CLASSNAME1 in ('" + CONT_CLASSNAME1 + "')")}
${if(len(CONT_CLASSNAME2) == 0,"","and CONT_CLASSNAME2 in ('" + CONT_CLASSNAME2 + "')")}

select distinct CONT_KIND_NAME FROM f_cost_adjcont

select distinct CONT_TYPE_NAME FROM f_cost_adjcont

select distinct CLOS_FLAG FROM f_cost_adjcont

select distinct CLOS_STAT_DESC FROM f_cost_adjcont

select CONT_ARCH_ID from f_cost_adjcont
where 1=1 
${if(len(AREA_ORG_NAME) == 0,"","and AREA_ORG_CODE in ('" + AREA_ORG_NAME + "')")}
${if(len(CITY_ORG_NAME) == 0,"","and CITY_ORG_CODE in ('" + CITY_ORG_NAME + "')")}
${if(len(PROJ_NAME) == 0,"","and PROJ_NAME in ('" + PROJ_NAME + "')")}
...
select CONT_NAME from f_cost_adjcont
where 1=1 
${if(len(AREA_ORG_NAME) == 0,"","and AREA_ORG_CODE in ('" + AREA_ORG_NAME + "')")}
${if(len(CITY_ORG_NAME) == 0,"","and CITY_ORG_CODE in ('" + CITY_ORG_NAME + "')")}
${if(len(PROJ_NAME) == 0,"","and PROJ_NAME in ('" + PROJ_NAME + "')")}
...
select PARTY_A_NAME from f_cost_adjcont
where 1=1
${if(len(AREA_ORG_NAME) == 0,"","and AREA_ORG_CODE in ('" + AREA_ORG_NAME + "')")}
${if(len(CITY_ORG_NAME) == 0,"","and CITY_ORG_CODE in ('" + CITY_ORG_NAME + "')")}
${if(len(PROJ_NAME) == 0,"","and PROJ_NAME in ('" + PROJ_NAME + "')")}
...
select PARTY_B_NAME from  f_cost_adjcont
where 1=1
${if(len(AREA_ORG_NAME) == 0,"","and AREA_ORG_CODE in ('" + AREA_ORG_NAME + "')")}
${if(len(CITY_ORG_NAME) == 0,"","and CITY_ORG_CODE in ('" + CITY_ORG_NAME + "')")}
${if(len(PROJ_NAME) == 0,"","and PROJ_NAME in ('" + PROJ_NAME + "')")}
...
select maintab.*,
subtab.ADJ_CONT_AMT,subtab.ADJ_CONT_CN,
subtab.APPROVAL_ADJCONT_CN,
subtab.APPROVAL_ADJCONT_AMT from
(
SELECT distinct 
${ if(INARRAY("1", SPLIT(show, ",")) = 0,"","AREA_ORG_NAME," ) } 
${ if(INARRAY("1", SPLIT(show, ",")) = 0,"","AREA_ORG_CODE," ) } 
...
'' as aa,CONT_AMT,REP_CONT_AMT,CURR_SIGN_AMT,CONT_NEW_AMT
FROM f_cost_adjcont
where 1=1 
${if(len(ARCHIVE_DATE1) == 0,"","and ARCHIVE_DATE >= '" + ARCHIVE_DATE1 + "'")}
${if(len(ARCHIVE_DATE2) == 0,"","and ARCHIVE_DATE <= '" + ARCHIVE_DATE2 + "'")}
...
select CONTRACT_NO from f_cost_adjcont
where 1=1 
${if(len(AREA_ORG_NAME) == 0,"","and AREA_ORG_CODE in ('" + AREA_ORG_NAME + "')")}
${if(len(CITY_ORG_NAME) == 0,"","and CITY_ORG_CODE in ('" + CITY_ORG_NAME + "')")}
${if(len(PROJ_NAME) == 0,"","and PROJ_NAME in ('" + PROJ_NAME + "')")}
