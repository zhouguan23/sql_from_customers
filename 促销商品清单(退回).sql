...
LEFT JOIN 
TB促销审核表 e on a.档期编码=e.档期编码 and a.商品编码=e.商品编码
left join 
tb职员用户表 f on e.职员编码=f.职员编码
...
,STUFF((SELECT distinct ',' + 促销部门 FROM #sj b WHERE a.档期编码=b.档期编码 and a.商品编码=b.商品编码  FOR XML PATH('')),1, 1, '') 部门,a.促销种类,a.补差方式,a.促销方式,a.批量数量,a.批量金额,a.原供价,a.原售价,a.特供价,a.特售价,a.满减金额,a.最大满减金额,sum(a.计划量)计划量,sum(a.计划额)计划额,sum(a.订货数量)订货数量,a.起始日期,a.结束日期,a.开始时间,a.结束时间,a.陈列位,a.状态,case when len(a.审批日期)=0 then '' else left(a.审批日期,4)+'年'+right(left(a.审批日期,6),2)+'月'+right(a.审批日期,2)+'日\n由 '+a.职员名称+' 退回\n原因：'+a.审批意见 end as 退回意见 from 
#sj a

group by  a.档期编码,a.主题名称,a.商品编码,a.商品名称,a.商品条码,a.单位,a.促销种类,a.补差方式,a.促销方式,a.批量数量,a.批量金额,a.原供价,a.原售价,a.特供价,a.特售价,a.满减金额,a.最大满减金额,a.起始日期,a.结束日期,a.陈列位,a.状态,a.开始时间,a.结束时间,case when len(a.审批日期)=0 then '' else left(a.审批日期,4)+'年'+right(left(a.审批日期,6),2)+'月'+right(a.审批日期,2)+'日\n由 '+a.职员名称+' 退回\n原因：'+a.审批意见 end
