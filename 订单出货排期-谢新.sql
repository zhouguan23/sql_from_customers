SELECT 
	...
	xmdd011 出货, xmda007 销退号,rtaxl003 产品类别
    FROM (select DISTINCT (case substr(xmdadocno,4 ,5)
		...
		xmdastus ,imaal003,xmdd001 , xmda022 ,pmab090,pmab091 ,pmab092,imafud001,ooag011,ooag003 ,xmdaent,xmdasite,imae034,imafud003,rtaxl003,
		...
		),0) l_m,(select rtaxl003 from imaa_t left join rtaxl_t on imaa009=rtaxl001 where imaa001=xmdd001 and rtaxl002='zh_CN' and rtaxlent=xmdaent) l_n
	FROM xmda_t@SOL  
		 ...
		 LEFT JOIN imaa_t@SOL ON imaa001=xmdd001 AND imaaent=xmddent 
		 LEFT JOIN rtaxl_t@SOL ON imaa009=rtaxl001 AND imaaent=xmddent AND rtaxl002='zh_CN' and rtaxlent=xmdaent
		 where 1=1 
and xmdaent = '100' and xmdasite ='SOL'  --and xmdastus != 'X'
and xmdastus ='Y'--not in('X','C') 19/12/3 xx改已审核
--and xmdd015=0 --销退数量
and xmdc045='1' --and xmdd001='111711028700'
...
${IF(LEN(comboCheckBox5) = 0, "", " AND rtaxl001 IN ('"+comboCheckBox5+"')")}
		) a1
...
,xmdd014,xmdasite,imae034,l_f,xmdd011,l_k,xmdadocnoj,l_j,xmda007,l_m,xmdaent,rtaxl003,
...
SELECT 
    ...
    FROM (select DISTINCT (case substr(xmdadocno,4 ,5)
		...
		LEFT JOIN xmdk_t ON xmdldocno=xmdkdocno AND xmdlent=xmdkent
		where xmdl008=xmdd001 AND xmdlent=xmdaent AND xmdl003 = xmdddocno and xmdl004=XMDDSEQ 
		and xmddseq=xmdl004 and xmddseq1=xmdl005 and xmddseq2=xmdl006 and xmdkstus  not in ('X','S') and xmdk026=xmdd011 and xmdk000 not in ('X','S')--add xmdk000 by xx 2020/04/10 出货单状态
		),0) l_m,(select rtaxl003 from imaa_t left join rtaxl_t on imaa009=rtaxl001 where imaa001=xmdd001 and rtaxl002='zh_CN' and rtaxlent=xmdaent) l_n
	FROM xmda_t  
		 ...
		 LEFT JOIN imaa_t ON imaa001=xmdd001 AND imaaent=xmddent 
		 LEFT JOIN rtaxl_t ON imaa009=rtaxl001 AND imaaent=xmddent AND rtaxl002='zh_CN' and rtaxlent=xmdaent
		 where 1=1 
...
AND ooac004 = '4') --AND xmdd001='111711028700'
and (xmdaud001 !='Y' or xmdaud001 is null)
...
order by 出货,单号
  ...
  select distinct imaal003,xmdd001,XMDA022 from xmda_t
		 LEFT JOIN xmdd_t ON xmddent = xmdaent AND xmdddocno = xmdadocno AND xmddsite = xmdasite 
...
select oocql002,oocql004 from oocql_t where oocql001='200' and oocqlent=98
