...
LEFT JOIN 
含税分课预算表 B ON b.DeptCode ='0002' and b.CategoryItemCode='0002' and BudgetYM =convert(varchar(6),dateadd(dd,0,'${dqrq}'),112)
...
set @jsny=@dqrq
	...
	SET @SQL=''
	SELECT @SQL=@SQL+' UNION ALL SELECT * FROM '+[name]A+' where  报表日期  = '+@dqrq+' and 商品分类项编码=''0000'' AND 商品分类编码 not LIKE ''1%'' AND 商品分类编码 not LIKE ''2%'' AND 商品分类编码 not LIKE ''6%'' AND 商品分类编码 not LIKE ''35000''' 
...
TB部门信息表 A,
 ('+@SQL+')  B
WHERE A.NodeCode=b.DeptCode  and  (nodecode LIKE ''1%'' or nodecode LIKE ''2%'') and goodscode like ''2%''
