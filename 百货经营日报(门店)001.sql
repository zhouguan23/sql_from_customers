...
and 1=1 ${if (and(left(right(CONCATENATE(GETUSERDEPARTMENTS()),4),1)<>'1' , left(right(CONCATENATE(GETUSERDEPARTMENTS()),4),1)<>'2') ,""," and nodecode =" + right(CONCATENATE(GETUSERDEPARTMENTS()),4) + "")}
...
select 
NodeCode,replace(NodeName,'服装','') NodeName,NodeName Node1,replace(NodeName,'服装','母婴') Node2,replace(NodeName,'母婴','儿童') Node3
,OpenDate,WhetherNew,AreaCode,AreaName,FormatCode,FormatName
 from 
dbo.TB部门信息表 a
