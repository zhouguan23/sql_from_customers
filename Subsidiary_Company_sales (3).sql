...
AND 
1=1
${if(len(SUBSIDIARY_COMPANY)=0,""," and A.AREA_CODE in ('"+SUBSIDIARY_COMPANY+"')")}
...
WHERE
A.AREA_CODE = B.AREA_CODE
AND SALE_DATE >= ADD_MONTHS(TO_DATE('${DATE1}','YYYY-MM-dd'),-12) AND  
SALE_DATE <= ADD_MONTHS(TO_DATE('${DATE2}','YYYY-MM-dd'),-12)
AND 
1=1
${if(len(SUBSIDIARY_COMPANY)=0,""," and A.AREA_CODE in ('"+SUBSIDIARY_COMPANY+"')")}
...
WHERE
A.AREA_CODE = B.AREA_CODE
AND SALE_DATE >= ADD_MONTHS(TO_DATE('${DATE1}','YYYY-MM-dd'),-1) AND  
SALE_DATE <= ADD_MONTHS(TO_DATE('${DATE2}','YYYY-MM-dd'),-1)
AND 
1=1
${if(len(SUBSIDIARY_COMPANY)=0,""," and A.AREA_CODE in ('"+SUBSIDIARY_COMPANY+"')")}
...
as 无税毛利率
FROM
DM_SHOP_COMPANY A,DIM_REGION B
WHERE
A.AREA_CODE = B.AREA_CODE
AND
--AREA_CODE = AREA_CODE
