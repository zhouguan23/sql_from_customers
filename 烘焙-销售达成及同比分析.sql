...
含税分课预算表
where(DeptCode   LIKE '1%' OR DeptCode   LIKE '2%')  and CategoryItemCode='0000' and( CategoryCode like '22%' OR CategoryCode like '282%'  OR CategoryCode like '289%')
...
and (CategoryCode like ''22%'' or CategoryCode like ''282%'' or CategoryCode like ''289%'')
...
(select nodecode ,sum(SaleIncome+saletax)SaleMoney,sum(TaxSaleGrossProfit)TaxSaleGrossProfit  from 
 ('+@SQL1+')  a,tb商品档案 b 
WHERE a.goodscode=b.goodscode  and  (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' ) 
and (CategoryCode like ''22%'' or CategoryCode like ''282%'' or CategoryCode like ''289%'')GROUP BY nodecode)b on a.nodecode= b.nodecode
left join 
(select DeptCode nodecode ,sum(SaleCount)SaleCount  from 
 ('+@SQL2+')  a
WHERE   (DeptCode LIKE ''1%'' OR DeptCode LIKE ''2%'' ) 
