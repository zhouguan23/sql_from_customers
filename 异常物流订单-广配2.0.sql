select h.oid as 物流订单ID,
 ...
 and org2.name like '广州配送中心%'
 and h.LOH_TYPE = 'collect_inbound'
 and h.status = 200
 and to_char(h.FORECASTTIME,'yyyy-mm-dd') <=
...
select h.oid as 物流订单ID,
 ...
 where h.OwnerId = '113003031'
 and org.name like '广州配送中心%'
 and (h.LOH_TYPE = 'outbound' or
      h.LOH_TYPE = 'outbound_deliver') 
 and h.status = 200
 and to_char(h.MODIFYTIME,'yyyy-mm-dd') <=
...
select h.oid as 物流订单ID,
 ...
 where h.OwnerId = '113003031'
 and org.name like '广州配送中心%'
 and (h.LOH_TYPE = 'outbound_transport_inbound' or
      h.LOH_TYPE = 'outbound_inbound') 
 and h.status = 200
 and to_char(h.MODIFYTIME,'yyyy-mm-dd') <=
     to_char(trunc(sysdate-2),'yyyy-mm-dd')
 and to_char(h.MODIFYTIME,'yyyy-mm-dd') >=
