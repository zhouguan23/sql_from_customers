...
(select distinct SX_ColonyCode ColonyCode,SX_ColonyName ColonyName from 
TB部门信息表 A 
where  (A.NodeCode LIKE '1%' OR A.NodeCode LIKE '2%') AND A.State!=2 AND LEN(SX_ColonyCode)!=0) b 
left join 
...
where DeptType='0' AND A.CategoryItemCode='0018' and  1=1 ${if(len(fl) == 0,   "",   "and left(b.CategoryCode,2) in ('" + replace(fl,",","','")+"')") }
