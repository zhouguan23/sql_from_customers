...
select item_id, sum(qty_ord)/5 qty  from  po   p,
 po_line   pl
 where p.po_num=pl.po_num and p.crt_date>='20130101' and crt_date<='20130630' 
group by item_id 
...
select item_id, sum(qty_ord)/5 qty  from  po   p,
 po_line   pl
 where p.po_num=pl.po_num and p.crt_date>='20130701' and crt_date<='20131231' 
group by item_id 

select item_id,kuaban from item_goujin where years='2013'
...
select a.item_id,sum(qty_sold*i.rods)/50000 qty from dpt_sale_day2013 a,item i
...
select i.item_id,sum(qty_sold*i.rods)/50000 benyue from dpt_sale_day2013 a,item i 
