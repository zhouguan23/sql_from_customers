...
and CategoryCode not in ('19','28','29')
...
select a.AreaCode ,a.nodecode,a.ParentCategoryCode,a.CategoryCode,isnull(b.StartCost,0)+isnull(c.Cost,0)StockCost,isnull(c.SaleCost,0)SaleCost from
...
when (left(CategoryCode,3) in (''280'',''287'') or left(CategoryCode,2)=''29'') then ''20'' 
   ...
   sum(PURCHCOST+ REDEPLOYINCOST+ PROFITCOST+ COUNTPROFITCOST - SaleCost  - REDEPLOYOUTCOST- LOSSCOST- COUNTLOSSCOST-ToGiftCost)COST,
   sum(case when a.nodecode in (6666,7777,8888) then 0 else   SaleCost end )SaleCost from 
 ('+@SQL1+') a ,tb商品档案 b 
where a.goodscode=b.goodscode 
and b.GoodsType not in (1,6,7)  
and  1=1 ${if(sx == 0,   "",   "and GoodsBrand=''" +sx+"''") }
