...
from (
select a.area_code,C.goods_code as goods_code,/*date1,*/(case when d.goods_code is null then '否' else '是' end) as dtp,case when nc.new_attribute is null then '地采' else nc.new_attribute end as JCSX,b.goods_name,b.manufacturer,b.specification,
...
and a.AREA_CODE=d.AREA_CODE and c.GOODS_CODE=d.GOODS_CODE
left join dim_net_catalogue_general_all nc on a.area_code=nc.area_code and c.goods_code=nc.goods_code and nc.create_month='${date1}'
where 1=1
...
group by a.area_code,C.goods_code,/*date1,*/(case when d.goods_code is null then '否' else '是' end),(case when nc.new_attribute is null then '地采' else nc.new_attribute end) ,b.goods_name,b.manufacturer,b.specification,b.CATEGORY,b.SUB_CATEGORY ) a,
...
where fs.AREA_CODE=dc.area_code
 ...
 where sale_data.goods_code=dg.goods_code(+)
   ...
   and sale_data.goods_code=zy_kc.goods_code(+)
      ...
      dim_goods_directory dgd/*,
      (select a.brancode,a.region,b.item,b.sale_price,b.business_status
 from zux_region_ou a ,cmx_item_area_attributes b where a.region=b.area_code and a.brancode=10) ciaa*/
 where jxc_data.goods_code=dgd.goods_code(+)
 and jxc_data.area_code=dgd.area_code(+)
