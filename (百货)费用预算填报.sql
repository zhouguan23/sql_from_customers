...
and left(deptcode,1) in (9) and CategoryCode not like '64%'
...
WHERE type = 'p'
    ...
    TB部门信息表 c 
    where left(NodeCode,1) in (9,5) and   1=1 ${if(len(bm) == 0,   "", "and c.NodeCode in ('" + replace(bm,",","','")+"')") })a
    left join 
...
select 
NodeCode,NodeCode+' '+replace(NodeName,'百货店' ,'鞋服百货店')Node
