...
WHERE type = 'p'
	...
	select a.ParentCategoryCode,a.CategoryCode ,a.CategoryName,YM month,a.NodeCode DeptCode ,
	sum(ISNULL(b.Salesindex,0))TAXSalesindex,sum(ISNULL(c.TAXSaleMoney,0))TAXSaleMoney,sum(ISNULL(c.AfterTAXSaleMoney,0))AfterTAXSaleMoney,
	sum(ISNULL(b.Grossprofitindex,0))TAXGrossprofitindex,sum(ISNULL(c.TAXGross,0)+isnull(e.kx,0))TAXGross,sum(ISNULL(c.AfterTAXGross,0))AfterTAXGross,
	sum(ISNULL(D.Salesindex,0))Salesindex,sum(ISNULL(c.SaleMoney,0))SaleMoney,sum(ISNULL(c.AfterSaleMoney,0))AfterSaleMoney,
	sum(ISNULL(D.Grossprofitindex,0))Grossprofitindex,sum(ISNULL(c.Gross,0)+isnull(e.kx,0))Gross,sum(ISNULL(c.AfterGross,0))AfterGross
	 from 
	(select b.YM,a.NodeCode,a.ParentCategoryCode,a.ParentCategoryName,a.CategoryCode,a.CategoryName  from 
	(select  b.NodeCode ,a.CategoryCode ParentCategoryCode,a.CategoryName ParentCategoryName,c.CategoryCode,c.CategoryName
	from 
	dbo.TB部门分类对照表 a 
	left join 
	dbo.TB部门分类对照表 c on a.deptcpde=c.deptcpde and a.CategoryCode=c.ParentCategoryCode
	,TB部门信息表 b 
	where a.CategoryItemCode='0000'  and a.deptcpde=b.FormatCode 
	and a.CategoryLevel=1
	...
	WHERE type = 'p'
	    ...
	    left join 
	    (select DeptCode,CategoryCode,BudgetYM,sum(Salesindex)Salesindex,sum(Grossprofitindex) Grossprofitindex
     from .[含税分课预算表]A
    where CategoryItemCode ='0000'  
    group by DeptCode,CategoryCode,BudgetYM) b on a.CategoryCode=b.CategoryCode and a.YM=b.BudgetYM and a.NodeCode=b.DeptCode 	       
    	    left join 
	    (select DeptCode,CategoryCode,BudgetYM,sum(Salesindex)Salesindex,sum(Grossprofitindex) Grossprofitindex
     from .[无税分课预算表]A
    where CategoryItemCode ='0000' 
    group by DeptCode,CategoryCode,BudgetYM) d on a.CategoryCode=d.CategoryCode and a.YM=d.BudgetYM and a.NodeCode=d.DeptCode 	     
    left join 
	    TB门店绩效表 c on a.CategoryCode=c.CategoryCode and a.YM=c.HappenYM and a.NodeCode=c.DeptCode and   c.CategoryItemCode ='0000' 
	        left join 
	    (select HappenYM,DeptCode,CategoryItemCode,CategoryCode,sum(ProcePayMoney)+sum(TaxRateDiff)+sum(LossProfitMoney)+sum(Maoriadjustmentoftheprocessing)+sum(Incomeadjustment)kx from 
	    TB门店绩效表毛利扣减表
	    where CategoryItemCode='0000' 
	    group by HappenYM,DeptCode,CategoryItemCode,CategoryCode)  e on a.CategoryCode=e.CategoryCode and a.YM=e.HappenYM and a.NodeCode=e.DeptCode 
        where 1=1 ${if(len(bm) == 0,   "",   "and a.nodecode in ('" + replace(bm,",","','")+"')") }  
        and 1=1 ${if(len(fl) == 0,   "",   "and a.ParentCategoryCode in ('" + replace(fl,",","','")+"')") } 
	    group by a.ParentCategoryCode,a.CategoryCode ,a.CategoryName,YM,a.NodeCode
...
from TB部门分类对照表
where CategoryItemCode='0000' and CategoryLevel='2' and DeptCpde = (select FormatCode  from TB部门信息表 a where a.nodecode ='${bm}')
...
select * from 
TB部门分类对照表
where CategoryLevel='1' and DeptCpde = (select FormatCode  from TB部门信息表 a where a.nodecode ='${bm}')
