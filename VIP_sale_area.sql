select  a.union_area_name,a.area_code,a.area_name,count(distinct b.cus_code) cus_code from dim_region a
left join dim_cus b
...
group by a.union_area_name,a.area_code,a.area_name

select A.area_code,count(distinct d.cus_code) cus_code,sum(A.tax_amount) as tax_amount,sum(tax_amount)-sum(A.tax_cost) as tax_profit,sum(origin_amount) origin_amount,count(distinct sale_date) sale_date
from dm_sale_tmp A
...
select t.area_code,sum(t.tran_num) tran_num from dm_transaction t
left join dim_member_day m
...
select distinct area_code,
                listagg(ddate, ',') within group(order by area_code) member_day
  from (select distinct area_code, to_char(ddate,'mm-dd') ddate
  from dim_member_day
 where ddate between date '${start}' and date '${end}')
 group by area_code


select area_code,count(distinct to_char(ddate,'mm-dd')) day_cnt from dim_member_day
