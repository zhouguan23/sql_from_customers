...
select r.union_area_name,sum(operating_profit) operating_profit,sum(employee_remuneration) employee_remuneration,sum(rental_fee) rental_fee,sum(subtotal_of_expenses) subtotal_of_expenses from fact_store_import a,dim_cus_transfer c,dim_region r
...
select  r.union_area_name,
 ...
 on a.area_code=c.area_code
 and a.cus_code=c.cus_code
 left  join dim_cus_transfer t
  on a.area_code=t.area_code
  and a.cus_code=t.cus_code
  
 where   sale_date between to_date('${start_date}','YYYY-MM')
and to_date('${end_date}','YYYY-MM')
and 1=1 ${if(len(UNION_AREA)=0,"","and r.UNION_AREA_NAME in ('"+UNION_AREA+"')")}
and t.cus_code is null

 and to_char(c.open_date,'yyyy')='${transfer_year}'
group by r.union_area_name,
  ...
  select union_area_name,
           case
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1<=12 then
              sum(sales_target_first)
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1 between 13 and 24 then
              sum(sales_target_first)+ sum(sales_target_second)/12*(months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))-11)
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1 >24 then
              sum(sales_target_first)+sum(sales_target_second)+sum(sales_target_third)/12*(months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))-23)
           end sales_traget,
           case
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1<=12 then
              sum(profit_forecast_first)
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1 between 13 and 24 then
              sum(profit_forecast_first)+ sum(profit_forecast_second)/12*(months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))-11)
             when months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))+1 >24 then
              sum(profit_forecast_first)+sum(profit_forecast_second)+sum(profit_forecast_third)/12*(months_between(to_date('${end_date}','yyyy-mm'),to_date(min(open_date),'yyyy-mm'))-23)
           end  profit_forecast
      from dim_cus_transfer t,dim_region r
      where   t.transfer_year='${transfer_year}'
      and t.area_code=r.area_code
