...
select AreaCode,a.NodeCode DeptCode,WhetherNew ,case when WhetherNew='1'  then '新店' when year='1'  then '开业1年以上' when year>'1'  then '开业2年以上' else '0' end year    ,
isnull(b.Salesindex,0) Salesindex,
isnull(b.Grossprofitindex,0) Grossprofitindex,
isnull(c.StartCost,0)+ isnull(StockCost,0) TaxCost
 from 
...
and (CategoryCode like '40%' or CategoryCode like '41%' or CategoryCode like '42%' 
  or CategoryCode like '43%')
...
where 1=1 ${if(len(nx) == 0,   "",   "and year in ('" + replace(nx,",","','")+"')") }
order by 1,4,3,2
...
and (CategoryCode like ''40%''  or  CategoryCode like ''41%'' or  CategoryCode like ''42%'' or  CategoryCode like ''43%'' or  CategoryCode like ''44%'' or  CategoryCode like ''45%'')
...
and (GoodsCatCode like ''40%''  or  GoodsCatCode like ''41%''  or  GoodsCatCode like ''42%''  or  GoodsCatCode like ''43%''  )
...
(select nodecode ,sum(SaleIncome+saletax)SaleMoney,sum(TaxSaleGrossProfit)TaxSaleGrossProfit  from 
 ('+@SQL1+')  a,tb商品档案 b 
WHERE a.goodscode=b.goodscode  and  (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' ) 
and (CategoryCode like ''40%''  or  CategoryCode like ''41%'' or  CategoryCode like ''42%'' or  CategoryCode like ''43%'')
and GoodsBrand =''010001''
GROUP BY nodecode)b on a.nodecode= b.nodecode
left join 
(select DeptCode nodecode ,sum(SaleCount)SaleCount  from 
