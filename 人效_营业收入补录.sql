...
and SMART_ORGNO_FULLPATH like '%50023968%' -- 取职能公司及子层级ID
...
and SMART_STATUS=1 -- 组织标识为有效
and SMART_CATEGORY in (2,3) -- 仅取最细粒度为公司级权限
and SMART_PARENTID = 50023968 -- 职能公司
${if(len(PARENT)==0," "," and SMART_PARENTID in ('" + PARENT + "')")}
ORDER BY SMART_ORGNO_FULLPATH

WITH T1 AS 
(select distinct year_half from dim_time
