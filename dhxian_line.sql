...
case status when '02' then '有效' when '03' then '暂停' else  '注销' end as status,b.qty_sold from co_cust c,dpt_sale dp,
 (
 select cust_id,avg(qty_sold) qty_sold from
 cust_sold  where   date1>='${date1}' and  date1<='${date2}' and 
  qty_sold>=(select avg(qty_sold)*5 from cust_sold where   date1>='${date1}' and  date1<='${date2}'  )
   group by cust_id
