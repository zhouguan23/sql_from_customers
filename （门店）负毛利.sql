...
(select a.OccurDate,a.NodeCode,left(b.CategoryCode,1)ParentCategoryCode,left(b.CategoryCode,2)CategoryCode,SUM(TaxSaleGrossProfit)TaxSaleGrossProfit,SUM(SaleIncome+SaleTax)SaleMoney from 
...
group by a.OccurDate,a.NodeCode,left(b.CategoryCode,1),left(b.CategoryCode,2))a
left join 
(select a.OccurDate,a.NodeCode,left(b.CategoryCode,1)ParentCategoryCode,left(b.CategoryCode,2)CategoryCode,
COUNT(a.GoodsCode)SKU,SUM(TaxSaleGrossProfit)TaxSaleGrossProfit,SUM(SaleIncome+SaleTax)SaleMoney from 
(select * from [000]A .tb${YM}_GoodsDayPSSM where round(TaxSaleGrossProfit,1)<0 and left(NodeCode ,1) between 5 and 9 and occurdate<='${rq}')  a,
[000]A .tbGoods b 
where a.GoodsCode =b.GoodsCode and b.GoodsType in (0,2) and left(b.CategoryCode,1) between 6 and 9 
and  1=1 ${if(len(fl2) == 0,   "",   "and left(b.CategoryCode,2)  in ('" + replace(fl2,",","','")+"')") }
and  1=1 ${if(len(bm) == 0,   "",   "and a.NodeCode in ('" + replace(bm,",","','")+"')") }
group by a.OccurDate,a.NodeCode,left(b.CategoryCode,1),left(b.CategoryCode,2))b on a.occurdate=b.occurdate and a.nodecode =b.nodecode and a.CategoryCode=b.CategoryCode
left join 
