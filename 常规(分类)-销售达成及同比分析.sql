...
datediff(day,OpenDate,convert(varchar(8),dateadd(d,-day('${tqdqrq}'),dateadd(m,1,'${tqdqrq}')),112))>=20  then 0 else 1 end  WhetherNew,
...
and CategoryItemCode='0000' and (CategoryCode like '3%' or CategoryCode like '4%' or CategoryCode like '5%')
...
set @qsny=@qsrq
SET @SQL1=''
SELECT @SQL1=@SQL1+' UNION ALL SELECT * FROM '+[name]A+' where occurdate between '+@qsrq+' and '+@dqrq+' and CategoryLevel=2 and CategoryItemCode  in(''0000'',''0001'') and (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' ) 
and (CategoryCode like ''3%'' or CategoryCode like ''4%'' or CategoryCode like ''5%'')
' 
 FROM SYS.SYSOBJECTS WHERE type='U' AND name LIKE '%_FLGoodsDAYpssm' and SUBSTRING(name,3,6) between  @qsny and @jsny
...
select a.AreaCode,b.CategoryCode,a.nodecode ,a.WhetherNew ,a.year,isnull(b.SaleMoney,0)SaleMoney,isnull(b.TaxSaleGrossProfit,0)TaxSaleGrossProfit,isnull(b.SaleCount,0)SaleCount from 
(select  AreaCode,NodeCode,case when 
datediff(day,OpenDate,convert(varchar(8),dateadd(d,-day(''${tqdqrq}''),dateadd(m,1,''${tqdqrq}'')),112))>=20
...
select nodecode  ,CategoryCode,sum(SaleIncome+saletax)SaleMoney,sum(TaxSaleGrossProfit)TaxSaleGrossProfit,sum(SaleCount)SaleCount  from 
 ('+@SQL1+')  a 

group by nodecode  ,CategoryCode)b on a.nodecode= b.nodecode
...
SET @SQL1=''
SELECT @SQL1=@SQL1+' UNION ALL SELECT * FROM '+[name]A+' where occurdate between '+@qsrq+' and '+@dqrq+' and CategoryLevel=2 and CategoryItemCode  in(''0000'',''0001'') and (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' ) 
and (CategoryCode like ''3%'' or CategoryCode like ''4%'' or CategoryCode like ''5%'' or CategoryCode like ''1%'' or CategoryCode like ''2%'')
' 
 FROM SYS.SYSOBJECTS WHERE type='U' AND name LIKE '%_FLGoodsDAYpssm' and SUBSTRING(name,3,6) between  @qsny and @jsny
