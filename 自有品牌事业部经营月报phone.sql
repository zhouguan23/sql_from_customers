...
TB部门信息表 A
LEFT JOIN 
含税分课预算表 B ON A.NodeCode=b.DeptCode and BudgetYM between convert(varchar(6),dateadd(dd,0,'${qsrq}'),112) and  convert(varchar(6),dateadd(dd,0,'${dqrq}'),112) and CategoryItemCode in ('0001') and CategoryCode not like '2%'
WHERE  (a.NodeCode LIKE '1%' or a.NodeCode LIKE '2%') and a.NodeCode not in ('1047')    
and A.OpenDate<=convert(varchar(6),dateadd(dd,0,'${qsrq}'),112)
