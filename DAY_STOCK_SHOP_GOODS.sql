...
(select distinct a.area_code,goods_code,dtp,gather,ddate from DAY_DM_STOCK_SHOP_DETAIL a,dim_cus dc
where 
  a.area_code=dc.area_code and a.cus_code=dc.cus_code
...
left join 
( select a.area_code,goods_code,sum(stock_qty) as stock_qty, sum(no_Tax_cost) as no_tax_cost
...
AND 1=1 ${if(len(dtp)=0,""," and dtp = '"+dtp+"'")}
AND 1=1 ${if(len(gather)=0,""," and gather in ('"+gather+"')")}
group by a.area_code,goods_code
) D on a.area_code=d.area_code and a.goods_code=d.goods_code
--- 当期过效期 


left join 
(select a.area_code,goods_Code, sum(no_tax_cost) as gxq_cost 
from DAY_DM_STOCK_SHOP_DETAIL a,dim_cus dc
where 
  a.area_code=dc.area_code and a.cus_code=dc.cus_code
AND 1=1 ${if(len(area)=0,""," and a.area_code in ('"+area+"')")} 
...
--- 当期近效期
left join 
(select a.area_code,goods_Code, sum(no_tax_cost) as jxq_cost 
from DAY_DM_STOCK_SHOP_DETAIL a,dim_cus dc
where 
  a.area_code=dc.area_code and a.cus_code=dc.cus_code
AND 1=1 ${if(len(area)=0,""," and a.area_code in ('"+area+"')")} 
...
select dr.area_code,dr.area_name from dim_region dr,USER_AUTHORITY  ua
