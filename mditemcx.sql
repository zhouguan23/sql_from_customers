...
select i.item_id,item_name,a.sale_dept_id,i.brdowner_id,
...
${if(len(sls)==0,""," and cc.slsman_id in ('"+sls+"')")}
  ...
  and   a.sale_dept_id in   ('${dptid}') 
and i.item_id in ('${itemid}')

and date1>='${date1}' and date1<='${date2}' 
group by i.item_id,item_name,a.sale_dept_id,i.brdowner_id 
...
and dp.sale_dept_id in ('${dptid}') order by dp.dpt_sale_id_new,slsman_id
