select  c.ChargeType,b.NodeCode ,c.CategoryCode CostSubjectCode,c.CategoryName CostSubjectName,a.CategoryCode ,a.CategoryName 
	from 
	dbo.TB部门分类对照表 a 
	,TB部门信息表 b 
	,TB费用科目表 c 
	where a.CategoryItemCode='0000'   and a.deptcpde=b.FormatCode 
	and a.CategoryLevel=1  and  c.CategoryItemCode in (select  distinct FormatCode from 
...
where  a.CategoryCode=b.ParentCategoryCode and  a.CategoryItemCode=b.CategoryItemCode)
and  a.CategoryItemCode in (select  distinct FormatCode from 
	TB部门信息表 where  1=1 ${if(len(bm) == 0,   "",   "and nodecode in ('" + replace(bm,",","','")+"')") })
union all 
select CategoryCode,CategoryName,ParentCategoryCode from 
tb费用科目表
where   CategoryItemCode in (select  distinct FormatCode from 
	TB部门信息表 where  1=1 ${if(len(bm) == 0,   "",   "and nodecode in ('" + replace(bm,",","','")+"')") })
)a
...
order by convert(int,CategoryCode),convert(int,ParentCategoryCode)
	...
	select *from 
	TB费用预算表
	where CategoryItemCode='0000' and BudgetYM ='201903' 
