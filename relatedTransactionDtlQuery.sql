...
SELECT DTL.ID,
 ...
 WHERE DTL.YEAR = NVL(SUBSTR('${YEARMONTHBEGIN}', 0, 4), DTL.YEAR)
   AND TO_NUMBER(DTL.MONTH) >=
       TO_NUMBER(NVL(SUBSTR('${YEARMONTHBEGIN}', 6, 2), DTL.MONTH))
   AND TO_NUMBER(DTL.MONTH) <=
       TO_NUMBER(NVL(SUBSTR('${YEARMONTHEND}', 6, 2), DTL.MONTH))
