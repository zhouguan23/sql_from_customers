...
(
SELECT 
SUM_FIXED_COST_UNEXP_AMT AS COST_1 -- 不含佣金
,SUM_FIXED_BDGET_AMT AS BDGET_COST_1 -- 预算值
,ORG_CODE -- 组织ID
FROM 
...
select 
LEFT(TMON,4) AS YEAR
,SUM(FIXED_COST_UNEXP_AMT) AS COST_2 -- 不含佣金
,SUM(FIXED_BDGET_AMT) AS BDGET_COST_2 -- 预算值
,ORG_CODE -- 组织ID
from
f_ehr_org_cost_d -- 成本及编制分析-组织成本执行情况表
...
(
SELECT 
FIXED_COST_UNEXP_AMT AS COST_3 -- 不含佣金
,FIXED_BDGET_AMT AS BDGET_COST_3 -- 预算值
,ORG_CODE -- 组织ID
FROM 
f_ehr_org_cost_d -- 成本及编制分析-组织成本执行情况表
WHERE TMON = '${pDATES}'
...
select 
LEFT(TMON,4) AS YEAR
,SUM(FIXED_COST_UNEXP_AMT) AS COST_4 -- 不含佣金
,SUM(FIXED_BDGET_AMT) AS BDGET_COST_4 -- 不含佣金预算值
,ORG_CODE -- 组织ID
from
f_ehr_org_cost_d -- 成本及编制分析-组织成本执行情况表
WHERE  LEFT (TMON,4) = LEFT('${pDATES}',4)
