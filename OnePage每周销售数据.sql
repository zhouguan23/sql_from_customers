...
select a.sd as name, a.year, a.[month]A, round(a.order_volume/1000,0) as ord_volume_kl, round(a.delivery_volume/1000,0) as del_volume_kl, round(a.c3/1000,0) as c3_k, b.[position]A, a.order_volume/b.total_volume_target as order_volume_phasing , a.delivery_volume/b.total_volume_target as del_volume_phasing, a.c3/b.total_c3_target as c3_phasing
from ((select sd, year, [month]A, sum(order_volume) as order_volume, sum(delivery_volume) as delivery_volume, sum(estimate_c3) as c3
    from report.onepage_bi_daily_sales_performance
    where year=year(getdate()) and month=month(getdate()-1)
    group by sd, year, [month]A) as a left join report.onepage_bi_daily_sales_performance_target as b
    on a.year=b.year and a.sd=b.name and b.position = 'SD')
where delivery_volume is not null and c3 is not null and order_volume is not null and  [name]A=N'${name}';

select a.sd as name, a.year, a.week_code, round(a.order_volume/1000,0) as ord_volume_kl, round(a.delivery_volume/1000,0) as del_volume_kl, round(a.c3/1000,0) as c3_k, b.[position]A, a.order_volume/b.total_volume_target as order_volume_phasing , a.delivery_volume/b.total_volume_target as del_volume_phasing, a.c3/b.total_c3_target as c3_phasing
from ((select sd, year, week_code, sum(order_volume) as order_volume, sum(delivery_volume) as delivery_volume, sum(estimate_c3) as c3
    from report.onepage_bi_daily_sales_performance
    where year=year(getdate()) and week_code=(datepart(week,getdate())-1)
    group by sd, year, week_code) as a left join report.onepage_bi_daily_sales_performance_target as b
    on a.year=b.year and a.sd=b.name and b.position = 'SD')
where delivery_volume is not null and c3 is not null and order_volume is not null and [name]A=N'${name}'
