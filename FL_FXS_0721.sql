...
WHERE
m.CLASS='FXS' and m.year=to_char(TO_DATE ('${start_date}', 'yyyy-mm'),'yyyy') 
...
from agreement m
left join region on m.regioncode=region.regioncode --省
where m.year=to_char(TO_DATE ('${start_date}', 'yyyy-mm'),'yyyy') and m.class='FXS'
...
FROM
  ...
  sdate,DAQU,DIQU,REGIONNAME,CITYNAME,SELLERID,ORGCODE,ORGNAME,CUSTOMTYPE,GRADE,PRODCODE,PRODID,PRDN,CREATEDATE,CREATEDATE,DATELASTUPDATED,
	...
	from V_STOCK where customtype='分销商'	
...
where  sorg.orgcode in ( SELECT DISTINCT SUPPLIER1_CODE FROM AGREEMENT@FR_LINK where class='FXS' ) 
or sorg.orgcode in (SELECT DISTINCT SUPPLIER2_CODE  FROM AGREEMENT@FR_LINK where class='FXS')
