select CategoryCode,sum(Salesindex)Salesindex,sum(Grossprofitindex)Grossprofitindex from 
含税分课预算表
where(DeptCode   LIKE '1%' OR DeptCode   LIKE '2%' OR DeptCode   LIKE '9%')  and CategoryItemCode='0001' and CategoryCode in (30,40,46,50)
...
select case 
    when  ( left(a.CategoryCode,2) In (''30'',''31'',''32'') )  then ''30''     
    when  ( left(a.CategoryCode,2) In (''14'',''33'',''34'') )  then ''33''     
    when  ( left(a.CategoryCode,2) in (''40'',''41'',''42'',''43'',''44'',''45'',''48'',''49''))  then ''40''     
    when  ( left(a.CategoryCode,2) in (''46'',''47''))  then ''46''     
    when  ( left(a.CategoryCode,2) in (''50'',''51'',''52''))  then ''50''    else left(a.CategoryCode,2) end  CategoryCode ,
    sum(a.SaleMoney)SaleMoney,sum(a.TaxSaleGrossProfit)TaxSaleGrossProfit,sum(b.SaleCount)SaleCount from 
(select left(b.CategoryCode,2)CategoryCode,sum(SaleIncome+saletax)SaleMoney,sum(TaxSaleGrossProfit)TaxSaleGrossProfit  from 
 ('+@SQL1+')  a,tb商品档案 b 
...
GROUP BY left(b.CategoryCode,2))a
left join 
(select GoodsCatCode CategoryCode,sum(SaleCount)SaleCount  from 
 ('+@SQL2+')  a
WHERE   (DeptCode LIKE ''1%'' OR DeptCode LIKE ''2%'' ) 
and 1=1 ${if(len(bm) == 0,   "",   "and DeptCode in (''" + replace(bm,",","'',''")+"'')") } 
GROUP BY GoodsCatCode)b on a.CategoryCode= b.CategoryCode
...
group by case 
    ...
    when  ( left(a.CategoryCode,2) in (''40'',''41'',''42'',''43'',''44'',''45'',''48'',''49''))  then ''40''     
    when  ( left(a.CategoryCode,2) in (''46'',''47''))  then ''46''     
    when  ( left(a.CategoryCode,2) in (''50'',''51'',''52''))  then ''50''    else left(a.CategoryCode,2) end 
