...
a.area_name,a.cus_code,c.UNION_AREA_NAME,c.sorted
order by c.sorted
...
select a.area_code,a.cus_code,
sum(TAX_AMOUNT) DIR_TAX_AMOUNT
from DM_SALE_TMP a,dim_cus b,dim_region c
where b.attribute='直营' and a.area_code=b.area_code and a.cus_code=b.cus_code
...
and ${if(len(cus) = 0, "1=1", "  a.cus_code in ('"+cus+"')") }
...
${if(len(chronic)=0,""," and a.ILL_TYPE in ('"+chronic+"')")}
${if(len(AREA)=0,""," and a.area_code in ('"+AREA+"')")}
${if(len(cus)=0,""," and a.cus_code in ('"+cus+"')")}
${if(len(UNION_AREA)=0,""," and c.union_area_name in ('"+UNION_AREA+"')")}
${if(len(attribute)=0,""," and b.attribute in ('"+attribute+"')")}
...
group by a.area_code,
a.area_name,a.INSIDERID,a.cus_code
)where TRAN_NUM>2
group by  area_code,
 area_name,cus_code
...
and ${if(len(AREA) = 0, "1=1", "  a.AREA_CODE in ('"+AREA+"')") }
${if(len(cus)=0,""," and a.cus_code in ('"+cus+"')")}
   and ${if(len(UNION_AREA) = 0, "1=1", "  c.union_area_name in ('"+UNION_AREA+"')")}
and dtp='否'
group by a.area_code,a.cus_code
