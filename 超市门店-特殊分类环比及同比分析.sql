...
and exists(select * from TB部门特殊商品对照 c where  b.nodecode=c.nodecode and c.GoodsPropertyCode='${sx}')
 )a
left join 
...
(select nodecode ,sum(SaleIncome+saletax)SaleMoney,sum(TaxSaleGrossProfit)TaxSaleGrossProfit  from 
 ('+@SQL1+')  a,tb商品档案 b 
WHERE a.goodscode=b.goodscode  and  (nodecode LIKE ''1%'' OR nodecode LIKE ''2%'' ) 
and exists(select * from TB部门特殊商品对照 c where a.goodscode=c.goodscode and a.nodecode=c.nodecode and c.GoodsPropertyCode=''${sx}'')
GROUP BY nodecode)b on a.nodecode= b.nodecode
left join 
(select DeptCode nodecode ,sum(SaleCount)SaleCount  from 
 ('+@SQL2+')  a
...
select a.AreaCode,a.nodecode ,a.WhetherNew,WhetherNew1 ,a.year,isnull(b.SaleMoney,0)SaleMoney,isnull(b.TaxSaleGrossProfit,0)TaxSaleGrossProfit from 
(select  AreaCode,NodeCode,case when 
datediff(day,OpenDate,''${tqdqrq}'')>=20
 then 0 else 1 end  WhetherNew  ,case when 
datediff(day,OpenDate,''${hqdqrq}'')>=20
 then 0 else 1 end  WhetherNew1,
 datediff(year,OpenDate,''${dqrq}'')year
from TB部门信息表 b
where  (NodeCode  LIKE ''1%'' OR NodeCode  LIKE ''2%'') AND nodecode<>''1047''
and OpenDate<=''${dqrq}'' )a
...
'exec(@sql)
 ...
 DECLARE @SQL VARCHAR(MAX),@SQL1 VARCHAR(MAX),@SQL2 VARCHAR(MAX),
@jsny varchar(6), @qsny varchar(6),@qsrq varchar(8),@dqrq varchar(8)
set @dqrq=convert(varchar(8),dateadd(dd,0,'${hqdqrq}'),112)
set @jsny=@dqrq
