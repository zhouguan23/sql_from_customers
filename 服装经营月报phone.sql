...
WHERE  (a.NodeCode LIKE '9%')  and a.nodecode not in ('9002','9021','9019','9999')
...
SELECT @SQL=@SQL+' UNION ALL SELECT * FROM '+[name]A+' where  报表日期 between  '+@qsrq+' and '+@dqrq+' and 商品分类项编码=''0000'' and 商品分类等级=0' 
 FROM SYS.SYSOBJECTS WHERE type='U' AND name LIKE '%_分类报表数据源' and left(CategoryCode,3) in ('610','611','620','621') and SUBSTRING(name,3,6) between  @qsny and @jsny
...
SELECT @SQL=@SQL+' UNION ALL SELECT * FROM '+[name]A+' where  BuildDate between  '+@qsrq+' and '+@dqrq+' ' 
 FROM SYS.SYSOBJECTS WHERE type='U' AND name LIKE '%_租赁商品销售明细' and left(CategoryCode,3) in ('610','611','620','621') and SUBSTRING(name,3,6) between  @qsny and @jsny
