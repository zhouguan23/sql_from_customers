...
select maintab.*
,subtab.CHG_CN
,subtab.APPROVAL_CHG_CN
,subtab.CHG_AMT
,subtab.CHG_EXCLSUB_AMT
from
(
select 
...
${if(len(CHG_CN)==0,"","and ifnull(CHG_CN,0)>0")}
...
sum(APPROVAL_CHG_CN) as APPROVAL_CHG_CN,
sum(CHG_AMT) as CHG_AMT,
sum(CHG_EXCLSUB_AMT) as CHG_EXCLSUB_AMT
from f_cost_vary_chg
where 1=1  
and area_org_code in(
select distinct b.AREA_ORG_CODE from user_org a  left join fr_po_priv b on a.SAP_DEPT_ID=b.proj_code
...
${ if(INARRAY("4", SPLIT(show, ",")) = 0,"","maintab.PROJ_CODE,STAGE_CODE," ) } 
2
