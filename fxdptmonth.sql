select  dpt_sale_id, i.kind,brand_id itemgr_id,IC.PRI_RTL,yieldly_type is_imported,ic.item_type1, i.is_thin,
 sum(qty_sold*i.t_size)/50000  qty,sum(amt_sold_with_tax) amt

from item_dpt_sale_month  a,plm_item i ,item_com ic,factory f  
where a.item_id=i.item_id and i.item_id=ic.item_id  
and f.fact_id=i.brdowner_id 
and date1>=(select to_char(to_date('${date1}','yyyymm'),'yyyymm')  from dual)
and date1<=(select to_char(to_date('${date2}','yyyymm'),'yyyymm')  from dual)
group by  
 dpt_sale_id, i.kind,brand_id  ,IC.PRI_RTL,yieldly_type  ,ic.item_type1, i.is_thin  having sum(qty_sold)<>0
...
and a.date1>='201801' and qty_sold<>0
) order by vend_id
...
from dpt_Sale_day2020 a,plm_item i  ,factory f
...
from dpt_Sale_day2019 a,plm_item i  ,factory f
where a.item_id=i.item_id and i.brdowner_id=f.fact_id 
 and iss_date<=(select to_char(add_months(sysdate,-12),'yyyymmdd') from dual)
group by brdowner_id,dpt_sale_id having sum(qty_sold)<>0
...
select brdowner_id,dpt_sale_id, 
 sum(qty_sold*i.t_size)/50000  mqty 
...
select brdowner_id,dpt_sale_id, 
 sum(qty_sold*i.t_size)/50000  mtqty 
