...
left join
(
select area_code,goods_code,insiderid,sale_date,sum(SALE_QTY) SALE_QTY,sum(TAX_AMOUNT) TAX_AMOUNT from
	(
	select area_code,goods_code,insiderid,SALE_QTY,TAX_AMOUNT,sale_date,
	ROW_NUMBER() OVER(partition by AREA_CODE,GOODS_CODE,insiderid order by sale_date desc nulls last) rn
	from
	sale 
	) a 
where rn=1
group by area_code,goods_code,insiderid,sale_date
)e on a.area_code=a.area_code and a.insiderid=e.insiderid and a.goods_code=e.goods_code
left join
(
select 
a.area_code,
...
a.area_code,
a.insiderid,a.goods_code
)f on a.area_code=f.area_code and a.insiderid=f.insiderid and a.goods_code=f.goods_code
