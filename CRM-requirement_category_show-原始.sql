...
SELECT categoryid as categoryid1, count(bugid) as allnum, 
sum(a.BUGKIND = '客户需求') as cdnum, 
count(distinct a.COMPANY) as ccnum, 
sum(POW(3,(a.BUGKIND = '客户需求') + (6-priority2) - 2) * (((a.BUGKIND = '客户需求') + 6 - priority2) > 1)) as pri,id as categoryid from 
(select t1.*,t2.*,ifnull(priority,3)priority2,c.id,name FROM fr_t_p_demand t1 left join fr_t_system1 t2 on (t1.bugid = t2.FR_SYSTEM_ID) 
left join fr_t_p_category c on concat(",",categoryid,",") regexp concat(",",c.id,",")

where (t2.bugstatus not in ('创建者验收','创建者补充','被否决','验收完成','已解决','关闭','BUG已解决') or t2.bugstatus is null)
and bugkind like '%需求'
...
${if(ifconsider=0," and (ifconsider is null or length(ifconsider)=0)","")})a
group by id 
order by pri
