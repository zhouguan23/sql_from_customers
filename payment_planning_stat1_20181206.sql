...
FROM(
SELECT * FROM(
SELECT s.sales_region,s.turn,new1.s.sales_name,ifnull(new1.amount,0)-ifnull(new2.amount,0) AS amount,((ifnull(new1.amount_this,0)-ifnull(new2.amount_this,0))-(ifnull(new1.amount_last,0)-ifnull(new2.amount_last,0)))/(ifnull(new1.amount_last,0)-ifnull(new2.amount_last,0)) AS this_last,ifnull(new1.amount_this,0)-ifnull(new2.amount_this,0) AS amount_this,ifnull(new1.amount_last,0)-ifnull(new2.amount_last,0) AS amount_last,(ifnull(new1.amount,0)-ifnull(new2.amount,0))/ifnull(new3.planning_value_e,0) AS rate,ifnull(new3.planning_value_e,0) AS planning_value_e,new1.year,
ifnull(new4.paid,0)-ifnull(new5.paid,0) AS amount_1,(ifnull(new4.paid,0)-ifnull(new5.paid,0))/ifnull(new3.planning_value_h,0) AS rate_1,ifnull(new3.planning_value_h,new3.planning_value_e*0.80) AS planning_value_h,if(length(planning_value_h)>0,1,0) AS isset
FROM
...
#and user_state <> '离职' 
WHERE length(sales_parent)>0 and sales_name in (SELECT distinct dist_salesman a FROM dict_district where dist_country ='中国' union select distinct user_username a from hr_user,hr_salesman where sales_name=user_username and user_state="离职" and ifnull(year(user_leaveDate),2016)>='${year}' and sales_region not in('台湾','日本','海外'))) AS s 
LEFT JOIN(

SELECT ctr_salesman,sum(if(year(ctr_signdate)='${year}',ctr_amount,0))-sum(if(year(ctr_signdate)='${year}',IFNULL(sumpay_bill,0),0)) AS amount,
sum(if(year(ctr_signdate)='${year}' AND ctr_signdate<=curdate(),ctr_amount,0))-sum(if(year(ctr_signdate)='${year}' AND ctr_signdate<=curdate(),IFNULL(sumpay_bill,0),0)) AS amount_this,sum(if(year(ctr_signdate)='${year}',0,ctr_amount))-sum(if(year(ctr_signdate)='${year}',0,IFNULL(sumpay_bill,0))) AS amount_last,'${year}' AS year
...
where (year(ctr_signdate)='${year}' OR ${if(year=year(today()),"(year(ctr_signdate)='"+(year-1)+"' AND ctr_signdate<='"+YEARDELTA(today(),-1)+"')","(year(ctr_signdate)='"+(year-1)+"')")}) and ctr_verified='valid' 
${if(year<=2017,"and ctr_status not in ('待审核','待录入','不合格','已作废')","and (ctr_status in ('已收回','已存档','订单已处理') or (ctr_status not in ('待审核','待录入','不合格','已作废','已收回','已存档') and sumpay_paid>0))")}
group by ctr_salesman,year) AS new1 on new1.ctr_salesman = s.sales_name
LEFT JOIN(

select ctr_salesman,sum(if(year(outs_recdate)='${year}',outs_amount,0)) amount,sum(if(year(outs_recdate)='${year}' AND outs_recdate<=curdate(),outs_amount,0)) amount_this,sum(if(year(outs_recdate)='${year}',0,outs_amount)) amount_last,'${year}' AS year
from sale_outsource
...
where (year(outs_recdate)='${year}' OR ${if(year=year(today()),"(year(outs_recdate)='"+(year-1)+"' AND outs_recdate<='"+YEARDELTA(today(),-1)+"')","(year(outs_recdate)='"+(year-1)+"')")})
and outs_status!="已作废"
group by ctr_salesman,year) AS new2
ON s.sales_name = new2.ctr_salesman
LEFT JOIN(

SELECT * FROM (select planning_id,planning_name,planning_year,(planning_value_e*f_amount)planning_value_e,planning_type,planning_currency,planning_value_h from sale_planning,finan_other_statistics where f_remark='CNY' and f_type=planning_currency)sale_planning 
WHERE planning_year='${year}'
) AS new3 ON s.sales_name = new3.planning_name
LEFT JOIN(

select pay_salesman,sum(pay_paid) paid,year(pay_enddate) year
from sale_payment 
...
group by pay_salesman,year(pay_enddate)) AS new4 ON new4.pay_salesman = new1.ctr_salesman
LEFT JOIN(
...
LEFT JOIN(
SELECT ctr_salesman,sum(outspay_paid) paid,year(outspay_paydate) year FROM `sale_outsource` a
...
and outs_status!="已作废"
group by ctr_salesman,year(outspay_paydate)) AS new5 ON new5.ctr_salesman = new1.ctr_salesman
...
) AS C ORDER BY ${ord} DESC) AS main,(SELECT @rank_o:=0,@rownum_o:=0) AS sub

SELECT type_region FROM dict_type WHERE type_region not in ("台湾","日本")

select *,round(planning_weifenpei/10000,2)weifenpei from sale_planning_of_region where planning_year ='${year}'and planning_weifenpei>0
...
LEFT JOIN hr_salesman_enter ON enter_user = sales_name JOIN hr_user on user_username = sales_name 
#and user_state <> '离职' 
WHERE length(sales_parent)>0 and sales_name in (select distinct user_username a from hr_user,hr_salesman where sales_name=user_username and user_state="离职" and ifnull(year(user_leaveDate),2016)>='${year}' and sales_region not in('台湾','日本','海外'))) AS s 
LEFT JOIN(
