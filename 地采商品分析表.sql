...
select  c.nodecode,sum(saleincome+ saletax)salemoney,sum(taxsalegrossprofit)taxsalegrossprofit    from TBGOODSPROPINCLUSIONS a ,TB商品档案 b ,tb${ym}_GoodsDayPSSM c  
 where GoodsPropertyCode ='2003'    and a.GoodsCode =b.GoodsCode and a.GoodsCode=c.GoodsCode and left(nodecode,1) in(1,2)   and b.CategoryCode not like '1%' and b.CategoryCode not like '2%'
 and c.occurdate='${dqrq}'
 group by  c.nodecode

select  c.nodecode,sum(saleincome+ saletax)salemoney,sum(taxsalegrossprofit)taxsalegrossprofit    from TBGOODSPROPINCLUSIONS a ,TB商品档案 b ,tb${ym}_GoodsDayPSSM c  
 where GoodsPropertyCode ='2003'    and a.GoodsCode =b.GoodsCode and a.GoodsCode=c.GoodsCode and left(nodecode,1) in(1,2) and b.CategoryCode not like '1%' and b.CategoryCode not like '2%'
 and c.occurdate between '${qsrq}' and '${dqrq}'
 group by  c.nodecode

select  c.nodecode,sum(saleincome+ saletax)salemoney,sum(taxsalegrossprofit)taxsalegrossprofit    from TB商品档案 b ,tb${ym}_GoodsDayPSSM c  
 where   b.GoodsCode=c.GoodsCode and left(nodecode,1) in(1,2)   and b.CategoryCode not like '1%' and b.CategoryCode not like '2%'
 and c.occurdate='${dqrq}'
 group by  c.nodecode
