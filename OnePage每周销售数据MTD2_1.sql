...
select sgm, year, month, sum(order_volume) as order_volume, sum(delivery_volume) as delivery_volume, sum(estimate_c3) as c3
    from report.onepage_bi_daily_sales_performance
    where year=year(getdate()) and month=month(getdate()-1) and date<=getdate()-1 and oil_level='PA'
    group by sgm, year, month
),
temp2 AS(
SELECT * from report.onepage_bi_daily_sales_performance_target 
...
select sgm, year, month, sum(order_volume) as order_volume, sum(delivery_volume) as delivery_volume
    from report.onepage_bi_daily_sales_performance
    where year=year(getdate()) and month=MONTH(getdate()-1)and date<=getdate()-1 and dpf1_group='Ultra'
    group by sgm, year, month
),
temp2 AS(
SELECT * from report.onepage_bi_daily_sales_performance_target 
...
select 
case when month(getdate())<'10' then '0'+cast(month(getdate())as varchar(50))+'-'+'01' else cast(month(getdate()) as varchar(50))+'-'+'01' end as firstday,
 right(getdate()-1,5) as lastday 
from report.onepage_bi_daily_sales_performance
