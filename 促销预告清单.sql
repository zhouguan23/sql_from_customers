...
where  
    1=1 ${if(len(dq) == 0,"","and a.档期编码 in ('" + replace(dq,",","','")+"')") }
...
and 1=1 ${if(len(sp) == 0,"","and a.商品编码 in ('" + replace(zp,",","','")+"')") }
and 1=1 ${if(len(fs) == 0,"","and a.补差方式 in ('" + replace(fs,",","','")+"')") }
group by 
a.档期编码,
a.主题名称,
a.商品编码,
