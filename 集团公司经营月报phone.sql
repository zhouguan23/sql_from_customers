...
from master..spt_values a ,TB部门信息表 b
where type='p' and number <= datediff(dd,dateadd(dd,-day('${dqrq}')+1,'${dqrq}')   ,dateadd(ms,-3,DATEADD(mm, DATEDIFF(m,0,'${dqrq}')+1, 0)))
and (NodeCode  LIKE '1%' OR NodeCode  LIKE '2%') AND NodeCode not in ('1047','1070')
...
where  a.defday=convert(varchar(8),dateadd(dd,0,'${dqrq}'),112) and a.NodeCode not in ('1047','1070')
...
where a.defday=convert(varchar(8),dateadd(dd,0,'${dqrq}'),112) and a.NodeCode not in ('1047','1070','1017')
